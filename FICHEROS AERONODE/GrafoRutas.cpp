...
//  Par auxiliar para Dijkstra: {costo, indice_nodo}
//  Usado en la cola de prioridad (min-heap manual con vector)
struct ParDijkstra {
    int  costo;
    int  indice;
};
...
std::vector<std::string> GrafoRutas::rutaMasCorta(const std::string& origen,
    ...
    for (int iter = 0; iter < n; iter++) {
        // Buscar el nodo no visitado con menor distancia (min-heap manual)
        int u = -1;
        for (int i = 0; i < n; i++) {
